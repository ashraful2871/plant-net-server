...
const corsOptions = {
  origin: [
    "http://localhost:5173",
    "http://localhost:5174",
    "https://planet-net-e7872.web.app",
    "https://planet-net-e7872.firebaseapp.com",
  ],
  credentials: true,
...
async function run() {
  try {
    ...
    //get all plants seller inventory
    ...
    //get all orders in specific seller  order data
    app.get(
      "/seller-orders/:email",
      verifyToken,
      verifySeller,
      async (req, res) => {
        ...
        const result = await ordersCollection
          .aggregate([
            {
              $match: { seller: email }, //match specific seller data only by email
            },
            {
              $addFields: {
                plantId: { $toObjectId: "$plantId" }, //convert plant id string field to objectId field
